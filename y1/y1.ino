...
Task t6(500, TASK_FOREVER, &t6Callback);
...
//เพื่อไว้ใส่โค้ดอุปกรณ์อื่นเพิ่ม
...
//วัดแกน    
    ...
    Wire.endTransmission(false);
    Wire.requestFrom(MPU_addr, 14, true); // request a total of 14 registers
      int t = AcX = Wire.read() << 8 | Wire.read(); 
      Serial.print("Acx t : ");
      Serial.println(t);    
      if (t > 10000) {
      DacAudio.Play(&CorceWithYou);
      }
  }
